...
COPY . /aioquic
RUN pip install --no-cache-dir --upgrade -r /aioquic/examples/conf_test3/requirements.txt
RUN pip install --no-cache-dir --upgrade -r /aioquic/requirements/doc.txt

RUN chmod u+x /aioquic/examples/conf_test3/client_startup.sh

WORKDIR /aioquic
RUN pip install -e .
ENV PYTHONPATH=/aioquic
#:/aioquic/src:/aioquic/src/aioquic
CMD ["./examples/conf_test3/client_startup.sh"]
